define(['../core'], function(c) {
  ...
  processResponse = function(resp, fields, seriesIdx) {
    ...
    if (chartType === 'scatter' && xEnum) {
      ...
      if (seriesList[1]) {
        formatterFunc = function() {
          return "<h5>" + this.series.name + "</h5><br /><b>" + xName + ":</b> " + this.series.xAxis.categories[this.point.x] + "<br /><b>" + yName + ":</b> " + (c.Highcharts.numberFormat(parseFloat(this.y)));
        };
      } else {
        formatterFunc = function() {
          return "<b>" + xName + ":</b> " + this.series.xAxis.categories[this.point.x] + "<br /><b>" + yName + ":</b> " + (c.Highcharts.numberFormat(parseFloat(this.y)));
        };
      }
    } else {
      if (seriesList[1]) {
        formatterFunc = function() {
          return "<h5>" + this.series.name + "</h5><br /><b>" + xName + ":</b> " + (c.Highcharts.numberFormat(parseFloat(this.x))) + "<br /><b>" + yName + ":</b> " + (c.Highcharts.numberFormat(parseFloat(this.y)));
        };
      } else {
        formatterFunc = function() {
          return "<b>" + xName + ":</b> " + (c.Highcharts.numberFormat(parseFloat(this.x))) + "<br /><b>" + yName + ":</b> " + (c.Highcharts.numberFormat(parseFloat(this.y)));
        };
      }
    }
    options = {
